...
export function AddSearchToFolderDialog({
  ...
  const t = useTranslations('addToFolder')
  ...
  const handleCreateAndAdd = async () => {
    ...
    if (ce || !folder) {
      setBusy(false)
      setError(ce?.message ?? t('createError'))
      return
    }
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 shadow-xl p-6 flex flex-col gap-4">
        ...
        {folders.length > 0 && (
          <div className="flex flex-col gap-2">
            <label className="text-xs font-medium text-neutral-700 dark:text-neutral-300">{t('existingFolder')}</label>
            <select
              value={folderId}
        ...
        <div className="border-t border-neutral-200 dark:border-neutral-800 pt-4 flex flex-col gap-2">
          <label className="text-xs font-medium text-neutral-700 dark:text-neutral-300">{t('newFolder')}</label>
          <input
            value={newName}
